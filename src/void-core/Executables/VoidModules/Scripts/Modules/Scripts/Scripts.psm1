...
# Backup Void Services and Drivers
$windir = $([Environment]::GetFolderPath('Windows'))
function Backup-VoidServices {
$filePath = "$([Environment]::GetFolderPath('Windows'))\VoidModules\Other\voidServices.reg"
...
& "$windir\VoidModules\Scripts\settingsPages.cmd" /hide account
...
function Disable-CoreIsolation {
    & ".\VoidModules\Scripts\ScriptWrappers\ConfigVBS.ps1" -DisableAllVBS
}
...
function Disable-Mitigations {
    Start-Process -FilePath "VoidDesktop\7. Security\Mitigations\Disable All Mitigations.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
}
...
# Set Void Default Profile Pictures
...
function Set-PowerSettings {
    ...
    if ($DisablePowerSaving) {
        Start-Process -FilePath "VoidDesktop\3. General Configuration\Power-saving\Disable Power-saving.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }

    if ($DisableHibernation) {
        Start-Process -FilePath "VoidDesktop\3. General Configuration\Hibernation\Disable Hibernation (default).cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }
